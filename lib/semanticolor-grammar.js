...
function semanticolorGrammarFactory(
	grammar,
	defaults,
	options,
	colorDiversity,
	...
	semanticolorGrammar.description =
		semanticolorGrammar.name +
		(utils.isTreeSitter(semanticolorGrammar)
			? ' (tree-sitter)'
			: ' (TextMate)');
	...
	if (utils.isTreeSitter(grammar)) {
		semanticolorGrammar.idForScope = function (scopeName, text) {
			...
			let { actions, includeAllChars } = determineActions(
				this.defaults,
				this.options,
				[scope],
			);
			let everythingElse =
				this.options.everythingElse === 'default'
					? this.defaults.everythingElse
					: this.options.everythingElse;
			let newScope = getNewScope(
				text,
				actions,
				this.colorDiversity,
				includeAllChars,
		...
		semanticolorGrammar.tokenizeLine = function (line, ruleStack, firstLine) {
			let tokenizeResult = this.__proto__.tokenizeLine(
				line,
				ruleStack,
				firstLine,
			);
			...
			let openScopeTokens =
				tokenizeResult.openScopeTokens &&
				tokenizeResult.openScopeTokens.slice();
			...
			if (tokenTagIndices.length) {
				for (let i = tokens.length - 1; i >= 0; i--) {
					...
					if (!includeAllChars && actions[actions.length - 1] === 'colorize') {
						subtokens = tokens[i].value
							.split(identifierChar)
							.filter((s) => s.length > 0);
					}
		...
		while (acts.length) {
			...
			switch (action) {
				...
				case 'contrast':
					if (
						includeAllChars ||
						identifierChar.test(str) ||
						defaultAction === 'contrast'
					) {
						newScope = contrast;
					} else {
						newScope = getNewScope(
							str,
							[defaultAction],
							colorDiversity,
							true,
				...
				case 'colorize-bg':
					...
					if (
						includeAllChars ||
						identifierChar.test(str) ||
						defaultAction === 'colorize'
					) {
						let token = str.trim();
						if (symbolPrefix.test(token)) {
							token = token.substring(1);
						}
						newScope = semanticolorScope(
							token,
							colorDiversity,
							action === 'colorize-bg',
						);

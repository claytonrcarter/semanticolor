	...
	activate: function () {
		...
		setTimeout(() => {
			...
			atom.grammars.forEachGrammar((g) => {
				...
				currentGrammars[getGrammarParamNameFromGrammarName(getGrammarName(g))] =
					g;
			});
		...
		function createGrammar(grammar) {
			...
			// FIXME is this wrong? how long do we need to support legacy ts parsers?
			if (
				(utils.isLegacyTreeSitter(grammar) &&
					!atom.config.get('core.useLegacyTreeSitter')) ||
				(utils.isWasmTreeSitter(grammar) &&
					atom.config.get('core.useLegacyTreeSitter'))
			) {
				return;
			}
			...
			const buildGrammar =
				!grammars[paramName] ||
				(utils.isWasmTreeSitter(grammar) &&
					utils.isLegacyTreeSitter(grammars[paramName]));

			if (options && buildGrammar) {
